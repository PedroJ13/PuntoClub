...
function createFakeRepository(options = {}) {
  ...
  return {
    ...
    async recordOperationalEmailAttempt(messageId, eventId, companyId, attempt) {
...
test('operational welcome email uses reply-to and customer recipient', () => {
  const message = createWelcomeEmail(sampleDetails, emailConfig, sampleSettings);
...
test('operational purchase email includes earned and total points', () => {
  const message = createPurchaseEmail(sampleDetails, emailConfig, sampleSettings);
