...
test("maps duplicate invoice SQL errors to API conflict", () => {
  const error = mapSqlError({
    number: 2601,
    message:
      "Cannot insert duplicate key row in object with unique index UX_Purchases_company_invoice.",
  });
...
test("maps duplicate customer SQL errors to API conflict", () => {
  const error = mapSqlError({
    number: 2627,
    message: "Violation of UNIQUE KEY constraint UX_Customers_company_phone.",
  });
  assert.equal(error.status, 409);
...
test("maps company email duplicate to company conflict", () => {
  const error = mapSqlError({
    number: 2601,
    message:
      "Cannot insert duplicate key row with unique index UX_Companies_email.",
  });
  assert.equal(error.status, 409);
...
test("maps pending registration duplicate to pending conflict", () => {
  const error = mapSqlError({
    number: 2601,
    message:
      "Cannot insert duplicate key row with unique index UX_CompanyRegistrationRequests_pending_company_email.",
  });
  assert.equal(error.status, 409);
...
test("maps pending invitation duplicate to pending conflict", () => {
  const error = mapSqlError({
    number: 2627,
    message:
      "Violation of UNIQUE KEY constraint UX_CompanyInvitations_pending_company_email.",
  });
  assert.equal(error.status, 409);
...
test("maps company user duplicates to user conflict", () => {
  const byEmail = mapSqlError({
    number: 2601,
    message:
      "Cannot insert duplicate key row with unique index UX_CompanyUsers_company_email.",
  });
  assert.equal(byEmail.status, 409);
  assert.equal(byEmail.code, "COMPANY_USER_ALREADY_EXISTS");

  const bySubject = mapSqlError({
    number: 2627,
    message: "Violation of UNIQUE KEY constraint UX_CompanyUsers_auth_subject.",
  });
...
test("maps transient SQL availability errors to service unavailable", () => {
  const unavailable = mapSqlError({
    code: "ELOGIN",
    message:
      "Database 'sql-db-puntoclub' on server 'sqlserver-pj13-brazil.database.windows.net' is not currently available.",
  });
  assert.equal(unavailable.status, 503);
  assert.equal(unavailable.code, "SERVICE_UNAVAILABLE");

  const firewallBlocked = mapSqlError({
    code: "ELOGIN",
    message:
      "Cannot open server 'sqlserver-puntoclub-stg-brazil' requested by the login. Client with IP address '203.0.113.10' is not allowed to access the server.",
  });
  assert.equal(firewallBlocked.status, 503);
  assert.equal(firewallBlocked.code, "SERVICE_UNAVAILABLE");

  const timeout = mapSqlError({
    code: "ETIMEOUT",
