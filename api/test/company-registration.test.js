...
test("getCompanyRegistrationReviewActorLabel defaults and trims configured actor", () => {
  ...
  assert.equal(
    getCompanyRegistrationReviewActorLabel({
      COMPANY_REGISTRATION_REVIEW_ACTOR_LABEL: " Admin API ",
    }),
    "Admin API",
  );
});
...
test("formatCompanyRegistrationLogoResponse returns private binary image response", () => {
  ...
  assert.equal(
    JSON.stringify(response).includes(
      "registration-requests/123/logo/private.png",
    ),
    false,
  );
});
...
test("getPromotionalEmailConfig prefers promotional sender with global fallback", () => {
  const config = getPromotionalEmailConfig({
    ACS_EMAIL_CONNECTION_STRING:
      "endpoint=https://example.communication.azure.com/;accesskey=secret",
    ACS_EMAIL_SENDER_ADDRESS: "operaciones@example.test",
    ACS_EMAIL_SENDER_DISPLAY_NAME: "Punto Club Operaciones",
    PROMOTIONAL_EMAIL_SENDER_ADDRESS: "campanas@example.test",
    PROMOTIONAL_EMAIL_SENDER_DISPLAY_NAME: "Punto Club Campañas",
    INTERNAL_NOTIFICATION_EMAIL: "ops@example.test",
  });

  assert.equal(config.enabled, true);
  assert.equal(config.senderAddress, "campanas@example.test");
  assert.equal(config.senderDisplayName, "Punto Club Campañas");

  const fallback = getPromotionalEmailConfig({
    ACS_EMAIL_CONNECTION_STRING:
      "endpoint=https://example.communication.azure.com/;accesskey=secret",
    ACS_EMAIL_SENDER_ADDRESS: "operaciones@example.test",
    ACS_EMAIL_SENDER_DISPLAY_NAME: "Punto Club Operaciones",
...
test("email templates use approved copy and escape HTML content", () => {
  ...
  assert.equal(
    internal.subject,
    "Punto Club - Nueva solicitud de empresa: <Cafe Central>",
  );
