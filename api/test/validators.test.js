...
test("customer payload normalizes optional email", () => {
  assert.deepEqual(
    ...
    { name: "Maria", phone: "8888", email: null, birthDate: null },
  );
});

test("normalizeEmail trims and lowercases email values", () => {
  assert.equal(
    normalizeEmail("  HOLA@CafeCentral.TEST "),
...
test("company registration request list query defaults and validates filters", () => {
  ...
  assert.deepEqual(
    validateCompanyRegistrationRequestListQuery(
      new URLSearchParams("status=all&limit=50"),
    ),
    { status: "all", limit: 50 },
  );
  ...
  assert.throws(
    () =>
      validateCompanyRegistrationRequestListQuery(
        new URLSearchParams("status=deleted&limit=100"),
      ),
    (error) =>
      error.status === 400 &&
...
test("company role and status helpers constrain known values", () => {
  ...
  assert.throws(
    () => validateCompanyRole("superadmin"),
    /One or more fields are invalid/,
  );
...
test("logo metadata validator accepts safe images and rejects svg or oversized files", () => {
  ...
  assert.throws(
    () =>
      validateLogoFileMetadata(
        { contentType: "image/svg+xml", size: 512, filename: "logo.svg" },
        { maxBytes: 1024 },
      ),
    /One or more fields are invalid/,
  );
  ...
  assert.throws(
    () =>
      validateLogoFileMetadata(
        { contentType: "image/jpeg", size: 2048, filename: "logo.jpg" },
        { maxBytes: 1024 },
      ),
    /One or more fields are invalid/,
  );
...
test("membership status query defaults and validates known statuses", () => {
  assert.deepEqual(validateMembershipStatusQuery(query({})), {
    status: "active",
  });
  assert.deepEqual(validateMembershipStatusQuery(query({ status: "all" })), {
    status: "all",
  });
...
test("customer membership status query supports lifecycle statuses", () => {
  assert.deepEqual(validateCustomerMembershipStatusQuery(query({})), {
    status: "active",
  });
  assert.deepEqual(
    validateCustomerMembershipStatusQuery(query({ status: "expired" })),
    { status: "expired" },
  );
  assert.deepEqual(
    validateCustomerMembershipStatusQuery(query({ status: "cancelled" })),
    { status: "cancelled" },
  );
  assert.deepEqual(
    validateCustomerMembershipStatusQuery(query({ status: "all" })),
    { status: "all" },
  );
...
test("expiration alerts query defaults and validates bounded window", () => {
  assert.deepEqual(validateExpirationAlertsQuery(query({})), {
    status: "active",
    withinDays: 5,
  });
  assert.deepEqual(
    validateExpirationAlertsQuery(
      query({ status: "expired", withinDays: "0" }),
    ),
    {
      status: "expired",
      withinDays: 0,
  ...
  assert.deepEqual(
    validateExpirationAlertsQuery(
      query({ status: "cancelled", withinDays: "30" }),
    ),
    {
      status: "cancelled",
      withinDays: 30,
  ...
  assert.throws(
    () =>
      validateExpirationAlertsQuery(
        query({ status: "all", withinDays: "366" }),
      ),
    /One or more fields are invalid/,
  );
});
...
test("membership benefit usage query validates bounded date range", () => {
  ...
  assert.throws(
    () =>
      validateMembershipBenefitUsageQuery(
        query({ from: "2026-06-13", to: "2026-06-01" }),
      ),
    /One or more fields are invalid/,
  );
  ...
  assert.throws(
    () =>
      validateMembershipBenefitUsageQuery(
        query({ from: "2026-06-01", to: "2026-07-02" }),
      ),
    /One or more fields are invalid/,
  );
});
...
test("activity report query defaults type to all", () => {
  ...
  assert.deepEqual(
    validateActivityReportQuery(
      query({ from: "2026-06-01", to: "2026-06-07", type: "membership" }),
    ),
...
test("activity report query validates type", () => {
  assert.throws(
    () =>
      validateActivityReportQuery(
        query({ from: "2026-06-01", to: "2026-06-07", type: "refund" }),
      ),
    /One or more fields are invalid/,
  );
});
...
test("customer report query validates search, type and bounded date range", () => {
  ...
  assert.deepEqual(
    validateCustomerReportQuery(
      query({ search: "+50688887777", from: "2026-06-01", to: "2026-06-07" }),
    ),
    {
      search: "+50688887777",
      from: "2026-06-01",
  ...
  assert.throws(
    () =>
      validateCustomerReportQuery(
        query({ search: "", from: "2026-06-01", to: "2026-06-07" }),
      ),
    /One or more fields are invalid/,
  );
  ...
  assert.throws(
    () =>
      validateCustomerReportQuery(
        query({ search: "Maria", from: "2026-06-01", to: "2026-07-02" }),
      ),
    /One or more fields are invalid/,
  );
});
...
test("audit events query validates allowed limits", () => {
  assert.deepEqual(
    validateAuditEventsQuery(
      query({ from: "2026-06-01", to: "2026-06-07", limit: "10" }),
    ),
  ...
  assert.throws(
    () =>
      validateAuditEventsQuery(
        query({ from: "2026-06-01", to: "2026-06-07", limit: "100" }),
      ),
    /One or more fields are invalid/,
  );
});
