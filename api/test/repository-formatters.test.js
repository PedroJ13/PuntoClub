...
test("toIsoTimestamp serializes Date values as complete UTC timestamps", () => {
  assert.equal(
    toIsoTimestamp(new Date("2026-06-02T15:20:00Z")),
...
test("mapCompanySettings exposes private logo route when blob metadata exists", () => {
  assert.deepEqual(
    mapCompanySettings({
      id: 8,
      name: "Aurisbel Pasteleria",
      email: "eventos.aurisbel@example.test",
      phone: "+50622223333",
      logo_url: null,
      logo_blob_path: "company-logos/8/logo.png",
      logo_content_type: "image/png",
      logo_updated_at: new Date("2026-07-07T18:30:00Z"),
      points_percentage: "5.00",
      loyalty_points_enabled: true,
      loyalty_memberships_enabled: true,
      status: "active",
      updated_at: new Date("2026-07-07T18:31:00Z"),
    }),
    {
      id: "8",
      name: "Aurisbel Pasteleria",
      ...
      logoUpdatedAt: "2026-07-07T18:30:00.000Z",
      pointsPercentage: 5,
      loyaltyPointsEnabled: true,
      loyaltyMembershipsEnabled: true,
      status: "active",
      updatedAt: "2026-07-07T18:31:00.000Z",
    },
  );
});
...
test("calculateExpirationAlert classifies date-only expiration windows", () => {
  ...
  assert.deepEqual(
    calculateExpirationAlert("2026-06-20", { today, warningDays: 5 }),
    {
      state: "none",
  ...
  assert.deepEqual(
    calculateExpirationAlert("2026-06-18", { today, warningDays: 5 }),
    {
      state: "expiring_soon",
  ...
  assert.deepEqual(
    calculateExpirationAlert("2026-06-13", { today, warningDays: 5 }),
    {
      state: "expires_today",
  ...
  assert.deepEqual(
    calculateExpirationAlert("2026-06-12", { today, warningDays: 5 }),
    {
      state: "expired",
...
test("calculateUsagePeriodStartDate derives ledger grouping dates", () => {
  assert.equal(
    calculateUsagePeriodStartDate("2026-06-17", "day", "2026-06-13"),
    "2026-06-17",
  );
  assert.equal(
    calculateUsagePeriodStartDate("2026-06-17", "week", "2026-06-13"),
    "2026-06-15",
  );
  assert.equal(
    calculateUsagePeriodStartDate("2026-06-21", "week", "2026-06-13"),
    "2026-06-15",
  );
  assert.equal(
    calculateUsagePeriodStartDate("2026-06-17", "month", "2026-06-13"),
    "2026-06-01",
  );
  assert.equal(
    calculateUsagePeriodStartDate(
