...
function formatPoints(value) {
  ...
  return new Intl.NumberFormat('es-CR', { maximumFractionDigits: 0 }).format(points);
}
...
function buildBaseMessage({ config, settings, customer, subject, plainText, html }) {
...
function createWelcomeEmail(details, config, settings) {
  ...
  const html = wrapOperationalEmailHtml('Bienvenido a Punto Club', [
  ...
  return buildBaseMessage({ config, settings, customer, subject, plainText, html });
}
...
function createPurchaseEmail(details, config, settings) {
  ...
  const html = wrapOperationalEmailHtml('Puntos ganados', [
    `<p>Hola ${notifier.escapeHtml(displayValue(customer.name))},</p>`,
...
function createRedemptionEmail(details, config, settings) {
  ...
  const html = wrapOperationalEmailHtml('Canje registrado', [
    `<p>Hola ${notifier.escapeHtml(displayValue(customer.name))},</p>`,
...
async function sendOperationalEmailBestEffort(repository, eventSpec, details, context, options = {}) {
  try {
    const { event, created } = await repository.createOperationalEmailEventIfNeeded(eventSpec);
    if (!created) {
      return { status: 'skipped', reason: 'duplicate_event', event };
    }

    const settings = await repository.getOperationalEmailSettings(eventSpec.companyId);
    const config = options.config || notifier.getEmailConfig(options.env || process.env);
    ...
    if (!isEventEnabled(eventSpec.eventType, settings)) {
      ...
      await repository.recordOperationalEmailAttempt(message.id, event.id, eventSpec.companyId, {
        provider: 'acs-email',
        status: 'skipped',
        reason: 'disabled_by_company_settings'
      });
      return { status: 'skipped', reason: 'disabled_by_company_settings', event };
    }
    ...
    const emailMessage = createMessage(eventSpec.eventType, details, config, settings);
    const message = await repository.createOperationalEmailMessage({
      eventId: event.id,
      companyId: eventSpec.companyId,
      customerId: eventSpec.customerId,
    ...
    try {
      const sendEmail = options.sendEmail || ((payload) => notifier.sendEmailViaAcs(payload, config, options));
      const result = await sendEmail(emailMessage);
      await repository.recordOperationalEmailAttempt(message.id, event.id, eventSpec.companyId, result);
      return { status: result.status, provider: result.provider, id: result.id || null, event };
    } catch (error) {
      const errorMessage = error && error.message ? error.message : 'Unknown email error.';
      warn(context, `Operational email was not sent: ${eventSpec.eventType}. ${errorMessage}`);
      await repository.recordOperationalEmailAttempt(message.id, event.id, eventSpec.companyId, {
        provider: 'acs-email',
        status: 'failed',
        errorMessage
      });
      return { status: 'failed', provider: 'acs-email', event };
    }
  } catch (error) {
    warn(context, `Operational email logging failed: ${eventSpec.eventType}. ${error && error.message ? error.message : 'Unknown email error.'}`);
