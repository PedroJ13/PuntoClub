...
Tarea completada: TASK-603 - Activar flag de envio promocional para prueba PO del nuevo flujo

Resultado:
- Se activo `PROMOTIONAL_EMAIL_SEND_ENABLED=true` en Azure Functions para la ventana controlada de prueba PO del nuevo flujo.
- Function App: `func-puntoclub-prod-br-001`.
- Resource group: `resource_group_main`.
- ACS Email esta configurado.
- No se cambio sender.
- No se cambiaron secretos.
- No se ejecuto envio promocional real desde esta tarea.
...
- Usar solo destinatarios seleccionados y controlados por PO desde el nuevo flujo.
...
- Si ACS falla o hay comportamiento inesperado, revertir el flag a `false` y crear tarea Backend/API o Infra segun corresponda.
